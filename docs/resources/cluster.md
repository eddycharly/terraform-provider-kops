...
- `legacy_cloud_provider` - (Optional) - String - The CloudProvider to use (aws or gce)<br />+k8s:conversion-gen=false.
...
- `subnets` - (Optional) - List([cluster_subnet_spec](#cluster_subnet_spec)) - Configuration of subnets we are targeting<br />+k8s:conversion-gen=false.
- `project` - (Optional) - String - Project is the cloud project we should use, required on GCE<br />+k8s:conversion-gen=false.
- `master_public_name` - (Optional) - String - MasterPublicName is the external DNS name for the master nodes<br />+k8s:conversion-gen=false.
- `master_internal_name` - (Optional) - String - MasterInternalName is unused.<br />+k8s:conversion-gen=false.
- `network_cidr` - (Optional) - String - NetworkCIDR is the CIDR used for the AWS VPC / GCE Network, or otherwise allocated to k8s<br />This is a real CIDR, not the internal k8s network<br />On AWS, it maps to the VPC CIDR.  It is not required on GCE.<br />+k8s:conversion-gen=false.
- `additional_network_cidrs` - (Optional) - List(String) - AdditionalNetworkCIDRs is a list of additional CIDR used for the AWS VPC<br />or otherwise allocated to k8s. This is a real CIDR, not the internal k8s network<br />On AWS, it maps to any additional CIDRs added to a VPC.<br />+k8s:conversion-gen=false.
- `network_id` - (Optional) - String - NetworkID is an identifier of a network, if we want to reuse/share an existing network (e.g. an AWS VPC)<br />+k8s:conversion-gen=false.
- `topology` - (Optional) - [topology_spec](#topology_spec) - Topology defines the type of network topology to use on the cluster - default public<br />This is heavily weighted towards AWS for the time being, but should also be agnostic enough<br />to port out to GCE later if needed<br />+k8s:conversion-gen=false.
...
- `additional_sans` - (Optional) - List(String) - AdditionalSANs adds additional Subject Alternate Names to apiserver cert that kops generates<br />+k8s:conversion-gen=false.
- `cluster_dns_domain` - (Optional) - String - ClusterDNSDomain is the suffix we use for internal DNS names (normally cluster.local).
- `service_cluster_ip_range` - (Optional) - String - ServiceClusterIPRange is the CIDR, from the internal network, where we allocate IPs for services<br />+k8s:conversion-gen=false.
- `pod_cidr` - (Optional) - String - PodCIDR is the CIDR from which we allocate IPs for pods<br />+k8s:conversion-gen=false.
- `non_masquerade_cidr` - (Optional) - String - MasterIPRange                 string `json:",omitempty"`<br />NonMasqueradeCIDR is the CIDR for the internal k8s network (on which pods & services live)<br />It cannot overlap ServiceClusterIPRange<br />+k8s:conversion-gen=false.
- `ssh_access` - (Optional) - List(String) - SSHAccess determines the permitted access to SSH<br />Currently only a single CIDR is supported (though a richer grammar could be added in future).
...
- `egress_proxy` - (Optional) - [egress_proxy_spec](#egress_proxy_spec) - HTTPProxy defines connection information to support use of a private cluster behind an forward HTTP Proxy<br />+k8s:conversion-gen=false.
- `ssh_key_name` - (Optional) - String - SSHKeyName specifies a preexisting SSH key to use.
- `kubernetes_api_access` - (Optional) - List(String) - KubernetesAPIAccess determines the permitted access to the API endpoints (master HTTPS)<br />Currently only a single CIDR is supported (though a richer grammar could be added in future)<br />+k8s:conversion-gen=false.
...
- `control_plane_kubelet` - (Optional) - [kubelet_config_spec](#kubelet_config_spec) - MasterKubelet is the kubelet configuration for nodes belonging to the control plane<br />It can be overridden by the kubelet configuration specified in the instance group.
...
- `node_termination_handler` - (Optional) - [node_termination_handler_spec](#node_termination_handler_spec) - NodeTerminationHandler determines the cluster autoscaler configuration.<br />+k8s:conversion-gen=false.
...
- `aws_load_balancer_controller` - (Optional) - [load_balancer_controller_spec](#load_balancer_controller_spec) - AWSLoadbalancerControllerConfig determines the AWS LB controller configuration.<br />+k8s:conversion-gen=false.
- `legacy_networking` - (Optional) - [networking_spec](#networking_spec) - Networking configuration<br />+k8s:conversion-gen=false.
- `networking` - (Required) - [networking_spec](#networking_spec)
- `legacy_api` - (Optional) - [api_spec](#api_spec) - API field controls how the API is exposed outside the cluster<br />+k8s:conversion-gen=false.
- `api` - (Optional) - [api_spec](#api_spec)
...
- `assets` - (Optional) - [assets](#assets) - Alternative locations for files and containers.
- `iam` - (Optional) - (Computed) - [iam_spec](#iam_spec) - IAM field adds control over the IAM security policies applied to resources.
- `encryption_config` - (Optional) - Bool - EncryptionConfig holds the encryption config.
- `tag_subnets` - (Optional) - Bool - DisableSubnetTags controls if subnets are tagged in AWS<br />+k8s:conversion-gen=false.
...
- `cluster_autoscaler` - (Optional) - [cluster_autoscaler_config](#cluster_autoscaler_config) - ClusterAutoscaler defines the cluaster autoscaler configuration.
- `warm_pool` - (Optional) - [warm_pool_spec](#warm_pool_spec) - WarmPool defines the default warm pool settings for instance groups (AWS only).<br />+k8s:conversion-gen=false.
...
- `pod_identity_webhook` - (Optional) - [pod_identity_webhook_spec](#pod_identity_webhook_spec) - PodIdentityWebhook determines the EKS Pod Identity Webhook configuration.<br />+k8s:conversion-gen=false.
...
- `control_plane` - (Required) - String - The environment to launch the Kubernetes masters in public|private.
...
- `dns` - (Required) - String
- `legacy_dns` - (Optional) - [dns_spec](#dns_spec) - DNS configures options relating to DNS, in particular whether we use a public or a private hosted zone<br />+k8s:conversion-gen=false.
...
- `idle_timeout_seconds` - (Optional) - Int - IdleTimeoutSeconds is unused<br />+k8s:conversion-gen=false.
...
- `additional_security_groups` - (Optional) - List(String) - AdditionalSecurityGroups is unused<br />+k8s:conversion-gen=false.
...
- `enable_etcd_tls` - (Optional) - Bool - EnableEtcdTLS is unused.<br />+k8s:conversion-gen=false.
- `enable_tls_auth` - (Optional) - Bool - EnableTLSAuth is unused.<br />+k8s:conversion-gen=false.
...
- `image` - (Optional) - String - Image is unused.<br />+k8s:conversion-gen=false.
- `replicas` - (Optional) - Int - Replicas is unused.<br />+k8s:conversion-gen=false.
...
- `default_not_ready_toleration_seconds` - (Optional) - Int - DefaultNotReadyTolerationSeconds.
- `default_unreachable_toleration_seconds` - (Optional) - Int - DefaultUnreachableTolerationSeconds.
...
- `attach_detach_reconcile_sync_period` - (Optional) - Duration - ReconcilerSyncLoopPeriod is the amount of time the reconciler sync states loop<br />wait between successive executions. Is set to 1 min by kops by default.
...
- `concurrent_rc_syncs` - (Optional) - Int - The number of replicationcontroller objects that are allowed to sync concurrently.<br />This only works on kubernetes >= 1.14.
...
- `use_service_account_credentials` - (Optional) - Bool - UseServiceAccountCredentials controls whether we use individual service account credentials for each controller.
- `enable_leader_migration` - (Optional) - Bool - EnableLeaderMigration enables controller leader migration.
- `cpu_request` - (Optional) - Quantity - CPURequest of CloudControllerManager container.<br />Default: 200m.
...
- `proxy_mode` - (Optional) - String - Which proxy mode to use: (userspace, iptables, ipvs).
...
- `gce_service_account` - (Optional) - String - GCEServiceAccount specifies the service account with which the GCE VM runs<br />+k8s:conversion-gen=false.
- `disable_security_group_ingress` - (Optional) - Bool - AWS cloud-config options.
- `elb_security_group` - (Optional) - String
- `v_sphere_username` - (Optional) - String - VSphereUsername is unused.<br />+k8s:conversion-gen=false.
- `v_sphere_password` - (Optional) - String - VSpherePassword is unused.<br />+k8s:conversion-gen=false.
- `v_sphere_server` - (Optional) - String - VSphereServer is unused.<br />+k8s:conversion-gen=false.
- `v_sphere_datacenter` - (Optional) - String - VShpereDatacenter is unused.<br />+k8s:conversion-gen=false.
- `v_sphere_resource_pool` - (Optional) - String - VSphereResourcePool is unused.<br />+k8s:conversion-gen=false.
- `v_sphere_datastore` - (Optional) - String - VSphereDatastore is unused.<br />+k8s:conversion-gen=false.
- `v_sphere_core_dns_server` - (Optional) - String - VSphereCoreDNSServer is unused.<br />+k8s:conversion-gen=false.
- `spotinst_product` - (Optional) - String - Spotinst cloud-config specs.
- `spotinst_orientation` - (Optional) - String
- `openstack` - (Optional) - [openstack_spec](#openstack_spec) - Openstack cloud-config options<br />+k8s:conversion-gen=false.
- `azure` - (Optional) - [azure_spec](#azure_spec) - Azure cloud-config options<br />+k8s:conversion-gen=false.
- `aws_ebs_csi_driver` - (Optional) - [ebs_csi_driver_spec](#ebs_csi_driver_spec) - AWSEBSCSIDriver is the config for the AWS EBS CSI driver<br />+k8s:conversion-gen=false.
- `gcp_pd_csi_driver` - (Optional) - [pd_csi_driver](#pd_csi_driver) - GCPPDCSIDriver is the config for the GCP PD CSI driver<br />+k8s:conversion-gen=false.
...
- `disable` - (Optional) - Bool - Disable indicates we do not wish to run the dns-controller addon<br />+k8s:conversion-gen=false.
...
- `tag_subnets` - (Optional) - Bool([Nullable](#nullable-arguments))
...
- `isolate_control_plane` - (Optional) - Bool
...
- `disable_tx_checksum_offloading` - (Optional) - Bool - DisableTxChecksumOffloading is unused.<br />+k8s:conversion-gen=false.
...
- `major_version` - (Optional) - String - MajorVersion is unused.<br />+k8s:conversion-gen=false.
...
- `flanneld_iptables_forward_rules` - (Optional) - Bool - DisableFlannelForwardRules configures Flannel to NOT add the<br />default ACCEPT traffic rules to the iptables FORWARD chain.
- `disable_tx_checksum_offloading` - (Optional) - Bool - DisableTxChecksumOffloading is unused.<br />+k8s:conversion-gen=false.
- `iptables_backend` - (Optional) - String - IptablesBackend controls which variant of iptables binary Felix uses<br />Default: Auto (other options: Legacy, NFT).
...
- `image` - (Optional) - String - ImageName is the container image name to use.
- `init_image` - (Optional) - String - InitImageName is the init container image name to use.
...
- `access_log` - (Optional) - String - AccessLog is unused.<br />+k8s:conversion-gen=false.
- `agent_labels` - (Optional) - List(String) - AgentLabels is unused.<br />+k8s:conversion-gen=false.
...
- `allow_localhost` - (Optional) - String - AllowLocalhost is unused.<br />+k8s:conversion-gen=false.
- `auto_ipv6_node_routes` - (Optional) - Bool - AutoIpv6NodeRoutes is unused.<br />+k8s:conversion-gen=false.
- `bpf_root` - (Optional) - String - BPFRoot is unused.<br />+k8s:conversion-gen=false.
...
- `container_runtime` - (Optional) - List(String) - ContainerRuntime is unused.<br />+k8s:conversion-gen=false.
- `container_runtime_endpoint` - (Optional) - Map(String) - ContainerRuntimeEndpoint is unused.<br />+k8s:conversion-gen=false.
- `debug` - (Optional) - Bool - Debug runs Cilium in debug mode.
- `debug_verbose` - (Optional) - List(String) - DebugVerbose is unused.<br />+k8s:conversion-gen=false.
- `device` - (Optional) - String - Device is unused.<br />+k8s:conversion-gen=false.
- `disable_conntrack` - (Optional) - Bool - DisableConntrack is unused.<br />+k8s:conversion-gen=false.
- `disable_endpoint_crd` - (Optional) - Bool - DisableEndpointCRD disables usage of CiliumEndpoint CRD.<br />Default: false.
- `disable_ipv4` - (Optional) - Bool - DisableIpv4 is unused.<br />+k8s:conversion-gen=false.
- `disable_k_8s_services` - (Optional) - Bool - DisableK8sServices is unused.<br />+k8s:conversion-gen=false.
...
- `enable_tracing` - (Optional) - Bool - EnableTracing is unused.<br />+k8s:conversion-gen=false.
...
- `envoy_log` - (Optional) - String - EnvoyLog is unused.<br />+k8s:conversion-gen=false.
...
- `ipv4_cluster_cidr_mask_size` - (Optional) - Int - Ipv4ClusterCIDRMaskSize is unused.<br />+k8s:conversion-gen=false.
- `ipv4_node` - (Optional) - String - Ipv4Node is unused.<br />+k8s:conversion-gen=false.
- `ipv4_range` - (Optional) - String - Ipv4Range is unused.<br />+k8s:conversion-gen=false.
- `ipv4_service_range` - (Optional) - String - Ipv4ServiceRange is unused.<br />+k8s:conversion-gen=false.
- `ipv6_cluster_alloc_cidr` - (Optional) - String - Ipv6ClusterAllocCidr is unused.<br />+k8s:conversion-gen=false.
- `ipv6_node` - (Optional) - String - Ipv6Node is unused.<br />+k8s:conversion-gen=false.
- `ipv6_range` - (Optional) - String - Ipv6Range is unused.<br />+k8s:conversion-gen=false.
- `ipv6_service_range` - (Optional) - String - Ipv6ServiceRange is unused.<br />+k8s:conversion-gen=false.
- `k_8s_api_server` - (Optional) - String - K8sAPIServer is unused.<br />+k8s:conversion-gen=false.
- `k_8s_kubeconfig_path` - (Optional) - String - K8sKubeconfigPath is unused.<br />+k8s:conversion-gen=false.
- `keep_bpf_templates` - (Optional) - Bool - KeepBPFTemplates is unused.<br />+k8s:conversion-gen=false.
- `keep_config` - (Optional) - Bool - KeepConfig is unused.<br />+k8s:conversion-gen=false.
- `label_prefix_file` - (Optional) - String - LabelPrefixFile is unused.<br />+k8s:conversion-gen=false.
- `labels` - (Optional) - List(String) - Labels is unused.<br />+k8s:conversion-gen=false.
- `lb` - (Optional) - String - LB is unused.<br />+k8s:conversion-gen=false.
- `lib_dir` - (Optional) - String - LibDir is unused.<br />+k8s:conversion-gen=false.
- `log_drivers` - (Optional) - List(String) - LogDrivers is unused.<br />+k8s:conversion-gen=false.
- `log_opt` - (Optional) - Map(String) - LogOpt is unused.<br />+k8s:conversion-gen=false.
- `logstash` - (Optional) - Bool - Logstash is unused.<br />+k8s:conversion-gen=false.
- `logstash_agent` - (Optional) - String - LogstashAgent is unused.<br />+k8s:conversion-gen=false.
- `logstash_probe_timer` - (Optional) - Int - LogstashProbeTimer is unused.<br />+k8s:conversion-gen=false.
- `masquerade` - (Optional) - Bool - DisableMasquerade disables masquerading traffic to external destinations behind the node IP.
- `nat46_range` - (Optional) - String - Nat46Range is unused.<br />+k8s:conversion-gen=false.
- `agent_pod_annotations` - (Optional) - Map(String) - AgentPodAnnotations makes possible to add additional annotations to the cilium agent.<br />Default: none.
...
- `pprof` - (Optional) - Bool - Pprof is unused.<br />+k8s:conversion-gen=false.
- `prefilter_device` - (Optional) - String - PrefilterDevice is unused.<br />+k8s:conversion-gen=false.
- `prometheus_serve_addr` - (Optional) - String - PrometheusServeAddr is unused.<br />+k8s:conversion-gen=false.
- `restore` - (Optional) - Bool - Restore is unused.<br />+k8s:conversion-gen=false.
- `single_cluster_route` - (Optional) - Bool - SingleClusterRoute is unused.<br />+k8s:conversion-gen=false.
- `socket_path` - (Optional) - String - SocketPath is unused.<br />+k8s:conversion-gen=false.
- `state_dir` - (Optional) - String - StateDir is unused.<br />+k8s:conversion-gen=false.
- `trace_payload_len` - (Optional) - Int - TracePayloadLen is unused.<br />+k8s:conversion-gen=false.
...
- `enable_ipv6` - (Optional) - Bool - EnableIpv6 is unused.<br />+k8s:conversion-gen=false.
- `enable_ipv4` - (Optional) - Bool - EnableIpv4 is unused.<br />+k8s:conversion-gen=false.
...
- `container_runtime_labels` - (Optional) - String - ContainerRuntimeLabels is unused.<br />+k8s:conversion-gen=false.
...
- `install_iptables_rules` - (Optional) - Bool - IPTablesRulesNoinstall disables installing the base IPTables rules used for masquerading and kube-proxy.<br />Default: false.
...
- `remove_cbr_bridge` - (Optional) - Bool - RemoveCbrBridge is unused.<br />+k8s:conversion-gen=false.
- `restart_pods` - (Optional) - Bool - RestartPods is unused.<br />+k8s:conversion-gen=false.
- `reconfigure_kubelet` - (Optional) - Bool - ReconfigureKubelet is unused.<br />+k8s:conversion-gen=false.
- `node_init_bootstrap_file` - (Optional) - String - NodeInitBootstrapFile is unused.<br />+k8s:conversion-gen=false.
- `cni_bin_path` - (Optional) - String - CniBinPath is unused.<br />+k8s:conversion-gen=false.
...
- `interval` - (Optional) - Int - Interval is publishing interval in minutes. This parameter is only used with classic load balancer.
- `bucket` - (Optional) - String - Bucket is S3 bucket name to store the logs in.
- `bucket_prefix` - (Optional) - String - BucketPrefix is S3 bucket prefix. Logs are stored in the root if not configured.
...
- `image` - (Optional) - String - Image is the AWS IAM Authenticator docker image to uses.
...
- `pod_annotations` - (Optional) - Map(String) - PodAnnotations are the annotations added to cluster autoscaler pod when they are created.<br />Default: none.
