...
func ResourceKopsKubeDNSConfig() *schema.Resource {
	return &schema.Resource{
		Schema: map[string]*schema.Schema{
			...
			"node_local_dns":       OptionalStruct(ResourceKopsNodeLocalDNSConfig()),
		},
	}
}

func ExpandResourceKopsKubeDNSConfig(in map[string]interface{}) kops.KubeDNSConfig {
	if in == nil {
		...
		NodeLocalDNS: func(in interface{}) *kops.NodeLocalDNSConfig {
			return func(in interface{}) *kops.NodeLocalDNSConfig {
				...
				return func(in kops.NodeLocalDNSConfig) *kops.NodeLocalDNSConfig {
					...
					return (ExpandResourceKopsNodeLocalDNSConfig(in.([]interface{})[0].(map[string]interface{})))
				}(in))
			}(in)
		}(in["node_local_dns"]),
	}
...
func FlattenResourceKopsKubeDNSConfigInto(in kops.KubeDNSConfig, out map[string]interface{}) {
		...
		return func(in *kops.NodeLocalDNSConfig) interface{} {
			...
			return func(in kops.NodeLocalDNSConfig) interface{} {
				return func(in kops.NodeLocalDNSConfig) []map[string]interface{} {
					return []map[string]interface{}{FlattenResourceKopsNodeLocalDNSConfig(in)}
				}(in)
			}(*in)
		}(in)
	}(in.NodeLocalDNS)
...
func FlattenResourceKopsKubeDNSConfig(in kops.KubeDNSConfig) map[string]interface{} {
	out := map[string]interface{}{}
	FlattenResourceKopsKubeDNSConfigInto(in, out)
	return out
}
