...
func ExpandResourceCiliumNetworkingSpec(in map[string]interface{}) kops.CiliumNetworkingSpec {
	...
	if in, ok := in["hubble"]; ok && in != nil {
		out.Hubble = func(in interface{}) *kops.HubbleSpec {
			...
			if in, ok := in.([]interface{}); ok && in != nil && len(in) == 1 {
				return func(in kops.HubbleSpec) *kops.HubbleSpec { return &in }(func(in interface{}) kops.HubbleSpec { return ExpandResourceHubbleSpec(in.(map[string]interface{})) }(in[0]))
			}
			return nil
		}(in)
	}
